...
$parentQuery = sqlsrv_query($conn, "SELECT fullName, userName, phoneNum FROM ParentTable");
while ($row = sqlsrv_fetch_array($parentQuery, SQLSRV_FETCH_ASSOC)) {
    $parents[] = $row;
}

$learnerQuery = sqlsrv_query($conn, "SELECT fullName, userName, phoneNum FROM LearnerTable");
...
<?php if ($role === 'teacher'): ?>
  ...
  <form method="POST" style="margin-bottom: 30px;">
    ...
    <label>Search Parent (Full Name):</label><br>
    <input list="parentList" id="parentFullName" oninput="fillParent(this.value)" style="width:100%; padding:8px;">
    <datalist id="parentList">
      <?php foreach ($parents as $p): ?>
        <option value="<?php echo htmlspecialchars($p['fullName']); ?>"></option>
      <?php endforeach; ?>
    </datalist>
